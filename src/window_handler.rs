//! Handling of [`WindowMessage`]s sent with
...
impl WindowMessage {
	/// Handle turning [`WindowMessage`] into a pointer and cleaning it up in
	/// case of an error.
...
thread_local! {
	...
	pub(crate) static WINDOW_HANDLER: MessageHandler = MessageHandler::new();
}
...
impl MessageHandler {
	...
	fn new() -> Self {
		...
		Self(Closure::wrap(Box::new(|event: &MessageEvent| {
			...
			// `JsValue`.
			#[allow(clippy::as_conversions)]
			let message = event.data().as_f64().expect("expected `f64`").to_bits() as *mut WindowMessage;
